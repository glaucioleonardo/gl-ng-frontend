...
export class GlComponentModalActionsService {
  ...
  addSingleAction(id: number, title: string, icon: string, action: () => void, red: boolean, green: boolean, hide?: boolean, groupSplit?: boolean, removeCurrent: boolean = false): void {
    ...
    if (!actionExists) {
      this.actions.push({ id, title, icon, action, red, green, hide, groupSplit });
    }
  }
